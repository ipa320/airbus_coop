...
class ssmGraph(object):
    def __init__(self, state_machine, action_server = None):
        ...
        self._graph_dot = pydot.Dot(graph_type = 'digraph', compound='true', newrank='true', minlen='0', size='"1,1"')
        self._level = 0
    ...
    def add_final(self, state, state_label, graph):
        for transition in state.get_internal_edges():
            if(transition[0] != 'preempt'):
                ...
                if(transition[1] in self._parallel_nodes):
                    node_name = transition[0]
                    self._parallel_ending.append('"'+transition[0]+'"')
                    graph.get_subgraph('cluster'+transition[1])[0].add_node(pydot.Node(node_name,shape='underline',margin='0',height='0.2',label=transition[0]))
