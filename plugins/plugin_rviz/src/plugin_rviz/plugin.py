...
#     def setup(self):
#         
#         config = self.get_param(name='yaml',
#                                 default="${prefix}/resources/config/default.rviz"
#                                 )
#         config = config.replace('${prefix}',get_pkg_dir('plugin_rviz'))
#         
#         self.my_rviz = MyViz(config)
#         
#         self.my_rviz.frame.centralWidget().setAttribute(Qt.WA_AcceptTouchEvents)
#         
#         self.set_widget(self.my_rviz)
#         
#     def user_changed_event(self, user):
#         
#         if user.privilege > Privilege.OPERATOR:
#             self.my_rviz.frame.setHideButtonVisibility( True )
#         else:
#             self.my_rviz.frame.setHideButtonVisibility( False )
    
# import sys
# import rospy
# if __name__ == "__main__":
#     a = QApplication(sys.argv)
#     rospy.init_node('plugin_rviz')
#     
#     from cobot_gui import load_plugin
#     
#     plugin = load_plugin('plugin_rviz', PluginRviz, 'plugin_rviz.xml')
#     
#     try:
#         MainWindow = plugin.get_widget()
#         MainWindow.show()
