...
from cobot_gui import plugin, ControlMode, EmergencyStopState, Alarm

from tuto_plugin.res import R

class TutoPlugin(plugin.Plugin):
    ...
    def onCreate(self, param):
        ...
        #self.title_label.setPixmap(R.getPixmapById("pere_noel2"))
        #self.title_label.setPixmap(QPixmap(R.images.pere_noel2)
        
        self.title_label.setStyleSheet(R.values.styles.hello)
        
        #with open(R.test.test1) as f:
         #   print f.read()
         
        #self.getContext().getLogger().err("In error")
        
        #data1 = param.getParam("data1", "default data 1")
        #data2 = float(param.getParam("data2", "-1"))
        #self.title_label.setText(data1)
    ...
    def onResume(self):
        ...
        #except Exception as ex:
            #self.getContext().sendAlarm(Alarm.ERROR,"%s on Resume exception %s"%(self.getPluginName(), str(ex)))
    ...
    def onTranslate(self, lng):
        ...
        """
        # Exemple:
        self.title_label.setText(R.values.strings.tuto(lng))
...
if __name__ == "__main__":
    ...
    rospy.init_node("tuto_plugin_node")
    
    a = QApplication(sys.argv)
    
    window = plugin.getStandAloneInstance("tuto_plugin", TutoPlugin, "en")
    window.setWindowTitle("TutoPlugin")
    window.show()
    a.exec_()
