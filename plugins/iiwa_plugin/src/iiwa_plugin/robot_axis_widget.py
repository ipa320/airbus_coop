...
#     RESOURCES_DIR = get_pkg_dir('plugin_robot_arm_control')+"/resources"
    ...
    def __init__(self, angle_max, torque_max = 110):
        QWidget.__init__(self)
        
        loadUi(R.layouts.robot_axis,self)
        
        self.min_angle_value.setText(str(-angle_max))
        self.max_angle_value.setText('+%s'%str(angle_max))
        
        self.px_min    = 0.
        self.px_max    = float(self.joint_angle_background.width())
        self.px_mid    = self.px_max/2
        self.angle_min = float(-angle_max)
        self.angle_max = float(angle_max)
        self.angle_a   = (self.px_max - self.px_min) / (self.angle_max - self.angle_min)
        self.angle_b   = self.px_max / 2.
        self.a_joint_handle  = 100./(self.angle_max - self.angle_min)
    ...
    def setJointAngle(self, angle):
        ...
        self.joint_angle_handle.move(int(self.angle_a*self.current_angle+self.angle_b)-int(self.a_joint_handle*self.current_angle+50.), 0)
